      ...
      tasks: [
        {
          ...
          description: 'Create a Dockerfile that uses Alpine Linux as base image and prints "Hello DevLab".\n\nUse the `echo` command to output the message.',
          hint: 'Use FROM alpine and CMD echo "Hello DevLab"',
          points: 25,
          checks: [
            { type: 'fileContains', file: 'Dockerfile', contains: 'FROM alpine' },
            { type: 'fileContains', file: 'Dockerfile', contains: 'Hello DevLab' }
          ]
        },
        {
          id: 'build_image',
          title: 'Build the Docker image',
          description: 'Build your Dockerfile into an image tagged as `devlab:1.0`.',
          hint: 'Use docker build -t devlab:1.0 .',
          points: 25,
          checks: [
            { type: 'commandExitCode', command: 'docker build -t devlab:1.0 .', expect: 0 }
          ]
        },
        {
          id: 'run_container',
          title: 'Run the container',
          description: 'Run the `devlab:1.0` image and verify it outputs "Hello DevLab".',
          hint: 'Use docker run --rm devlab:1.0',
          points: 25,
          checks: [
            { type: 'commandExitCode', command: 'docker run --rm devlab:1.0 | grep "Hello DevLab"', expect: 0 }
          ]
        },
        {
          id: 'list_images',
          title: 'List Docker images',
          description: 'Use Docker commands to list all images and confirm `devlab:1.0` exists.',
          hint: 'Use docker images or docker image ls',
          points: 25,
          checks: [
            { type: 'commandExitCode', command: 'docker images devlab:1.0 | grep "devlab"', expect: 0 }
          ]
        }
      ]
    })
      ...
      tasks: [
        {
          ...
          description: 'Create a new Git repository and add an initial README file.',
          hint: 'Use git init, create README.md, then git add and git commit',
          points: 20,
          checks: [
            { type: 'commandExitCode', command: 'test -d .git', expect: 0 },
            { type: 'fileContains', file: 'README.md', contains: 'DevLab' }
          ]
        },
        {
          id: 'create_branch',
          ...
          points: 20,
          checks: [
            { type: 'commandExitCode', command: 'git branch | grep "feature/login"', expect: 0 }
          ]
        },
        {
          id: 'make_changes',
          title: 'Make changes and commit',
          description: 'Create a file called `login.js` with some JavaScript code and commit it.',
          hint: 'Create the file, git add, then git commit',
          points: 30,
          checks: [
            { type: 'fileContains', file: 'login.js', contains: 'function' },
            { type: 'commandExitCode', command: 'git log --oneline | grep -i login', expect: 0 }
          ]
        },
        {
          id: 'merge_branch',
          title: 'Merge feature branch',
          description: 'Switch back to main branch and merge the feature branch.',
          hint: 'git checkout main, then git merge feature/login',
          points: 30,
          checks: [
            { type: 'commandExitCode', command: 'git log --oneline | wc -l | grep -E "^[2-9]"', expect: 0 }
          ]
        }
      ]
    })
...
By the end of this lab, you'll be proficient with core Linux commands used in DevOps workflows.`,
    yamlSpec: JSON.stringify({
      environment: {
        ...
        setup: ['apt update && apt install -y bash curl wget']
      },
      tasks: [
        {
          id: 'navigate_filesystem',
          title: 'Navigate the filesystem',
          description: 'Navigate to the `/tmp` directory and create a directory called `devlab`.',
          hint: 'Use cd /tmp && mkdir devlab',
          points: 25,
          checks: [
            { type: 'commandExitCode', command: 'test -d /tmp/devlab', expect: 0 }
          ]
        },
        {
          id: 'create_files',
          title: 'Create and edit files',
          description: 'In the devlab directory, create a file called `servers.txt` with three server names (one per line).',
          hint: 'Use echo or cat with redirection',
          points: 25,
          checks: [
            { type: 'fileContains', file: '/tmp/devlab/servers.txt', contains: 'server' },
            { type: 'commandExitCode', command: 'wc -l /tmp/devlab/servers.txt | grep "3"', expect: 0 }
          ]
        },
        {
          id: 'file_permissions',
          title: 'Set file permissions',
          description: 'Make the servers.txt file readable and writable by owner only (600).',
          hint: 'Use chmod 600 /tmp/devlab/servers.txt',
          points: 25,
          checks: [
            { type: 'commandExitCode', command: 'ls -l /tmp/devlab/servers.txt | grep "^-rw-------"', expect: 0 }
          ]
        },
        {
          id: 'find_files',
          title: 'Search for files',
          description: 'Use find command to locate all .txt files in the /tmp directory.',
          hint: 'Use find /tmp -name "*.txt"',
          points: 25,
          checks: [
            { type: 'commandExitCode', command: 'find /tmp -name "*.txt" | grep servers.txt', expect: 0 }
          ]
        }
      ]
    })
